...
const struct maybe_array_int none_array_int = {
    {NULL, 0},
    false
};
...
bool array_array_int_set(struct array_array_int a, size_t i, size_t j,
                         int64_t value) {
    struct maybe_array_int row = array_array_int_get_row(a, i);
    if (row.valid && array_int_set(row.value, j, value) &&
        array_array_int_set_row(a, i, row.value)) {
        return true;
    } else {
        return false;
    }
...
void array_array_int_normalize(struct array_array_int array, int64_t m) {
    for (size_t i = 0; i < array.size; i = i + 1) {
        const struct maybe_array_int cur_row =
            array_array_int_get_row(array, i);
        if (cur_row.valid) {
